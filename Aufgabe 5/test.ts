...
namespace aufgabe4 {
    ...
    function init(_event: Event): void {
        ...
        drawZufallsBlumen();
        //Hintergrund Speichen für Animation
...
//gespeicherter Hintergrund anzeigen
        crc2.putImageData(hintergrund, 0, 0);
//grobe Flugrichtung links
        for (let i: number = 0; i < n; i++) {
            if (button == true) {
                x[i] += Math.random() * (- 2);
...
//Biene am anderen Bildrand auftauchen lassen
            if (x[i] < 0){
                x[i] = 350;
                }
            if (x[i] >= 350){
...
//        crc2.moveTo(_x, _y - 2);
//        crc2.arc(_x, _y - 2, 3.5, 1.5 * Math.PI, 0.5 * Math.PI)
//        crc2.fill();
...
//        crc2.moveTo(_x, _y);
//        crc2.arc(_x, _y, 3.5, 0.5 * Math.PI, 1.5 * Math.PI);
//        crc2.moveTo(_x - 3.5, _y - 2);
//        crc2.lineTo(_x - 3.5, _y - 2);
//        crc2.moveTo(_x, _y - 2);
//        crc2.arc(_x, _y - 2, 3.5, 0.5 * Math.PI, 1.5 * Math.PI);
//        crc2.fill();
    }
